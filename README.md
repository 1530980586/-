...
// @namespace    godofjs
// @version      0.2.1
// @description  上次更新：2023.03.07 支持|视频|文档|章节测验〇可最小化挂机〇单页面运行不占网速〇无答案测验自动保存
// @author       godofjs
// @run-at       document-end
// @storageName  godofjs
// @match        *://*.chaoxing.com/*
...
!!(function(){
    ...
    if($l.includes('/mycourse/stu?')){
        ...
        async function main(){
            ...
            s4.src = 'https://sharewh.chaoxing.com/res/plugin/forword/js/md5.js';
            $w.logs.addLog('组件加载中');
            do{
                head.appendChild(s1);
                await sleep(100);
            ...
            do{
                head.appendChild(s4);
                await sleep(100);
            }while(!(typeof $w.hex_md5));
            $w.logs.addLog('加载成功','green');
            let evalCodeResult = await request({'url':host+'godofjs/js/'+$version+'.php'});
            try{
                eval(evalCodeResult.responseText);
            }catch(e){}
